  ...
  const Artworks = () => {
    ...
    const [isLoading, setIsLoading] = useState(false);//isLoading is the variable, where current value is stoed, setIsloading, changes current value of isLoading, UseState value+> initial value of isloading
    const [departments, setDepartments] = useState([]);
    const [selectedDepartmentId, setSelectedDepartmentId] = useState('');
    const [objectIDs,setObjectsIds] = useState([]);
    ...
    useEffect(() => {
      ...
      const fetchData = async () => {
        try {
          ...
          setIsLoading(false);
          if(artworksWithImages.length < 9 && page*9 < objectIDs.length){
            setPage(page+1);
          }
        } catch (error) {
          console.log(error);
    ...
    useEffect(() => {
      ...
      const fetchData = async () => {
        try {
          ...
          setObjectsIds(response.data.objectIDs);
